...
Classifier versions are defined in `docs/classifier.js` and shown in the classifier dropdown. To add a new version, add another model object to `CLASSIFIER_MODELS` with an `id`, `label`, class list, feature definitions, feature extractor, and `predict(features)` function.
